...
public class CameraControllerWOW : MonoBehaviour
{
    ...
    // Lock camera to rear of target
    public bool allowMouseInputX = true;
    // Allow player to control camera angle on the X axis (Left/Right)
    public bool allowMouseInputY = true;
    // Allow player to control camera angle on the Y axis (Up/Down)
    ...
    void LateUpdate()
    {
        ...
        //Sidebuttonmovement
        if ((CnInputManager.GetAxis("Horizontal") != 0) && (pbuffer == 0))
        {
            pbuffer = coolDown;
            mouseSideButton = !mouseSideButton;
        }
        if (mouseSideButton && CnInputManager.GetAxis("Vertical") != 0)
